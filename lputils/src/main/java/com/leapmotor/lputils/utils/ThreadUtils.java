...
public class ThreadUtils {
    ...
    private static final Map<ThreadUtils.Task, ExecutorService> TASK_POOL_MAP = new ConcurrentHashMap<>();
     ...
     */
    public static <T> void executeByFixed(@IntRange(from = 1) final int size, final ThreadUtils.Task<T> task) {
     ...
     * @param <T>  The type of the task's result.
     */
    public static <T> void executeBySingle(final ThreadUtils.Task<T> task) {
     ...
     * @param <T>   The type of the task's result.
     */
    public static <T> void executeBySingleWithDelay(final ThreadUtils.Task<T> task,
                                                    final long delay,
                                                    final TimeUnit unit) {
     ...
     * @param <T>    The type of the task's result.
     */
    public static <T> void executeBySingleAtFixRate(final ThreadUtils.Task<T> task,
                                                    final long period,
                                                    final TimeUnit unit) {
     ...
     * @param <T>  The type of the task's result.
     */
    public static <T> void executeByCached(final ThreadUtils.Task<T> task) {
     ...
     * @param <T>   The type of the task's result.
     */
    public static <T> void executeByCachedWithDelay(final ThreadUtils.Task<T> task,
                                                    final long delay,
                                                    final TimeUnit unit) {
     ...
     * @param <T>    The type of the task's result.
     */
    public static <T> void executeByCachedAtFixRate(final ThreadUtils.Task<T> task,
                                                    final long period,
                                                    final TimeUnit unit) {
     ...
     * @param <T>  The type of the task's result.
     */
    public static <T> void executeByIo(final ThreadUtils.Task<T> task) {
     ...
     * @param <T>   The type of the task's result.
     */
    public static <T> void executeByIoWithDelay(final ThreadUtils.Task<T> task,
                                                final long delay,
                                                final TimeUnit unit) {
     ...
     * @param <T>    The type of the task's result.
     */
    public static <T> void executeByIoAtFixRate(final ThreadUtils.Task<T> task,
                                                final long period,
                                                final TimeUnit unit) {
     ...
     * @param <T>  The type of the task's result.
     */
    public static <T> void executeByCpu(final ThreadUtils.Task<T> task) {
     ...
     * @param <T>   The type of the task's result.
     */
    public static <T> void executeByCpuWithDelay(final ThreadUtils.Task<T> task,
                                                 final long delay,
                                                 final TimeUnit unit) {
     ...
     * @param <T>    The type of the task's result.
     */
    public static <T> void executeByCpuAtFixRate(final ThreadUtils.Task<T> task,
                                                 final long period,
                                                 final TimeUnit unit) {
     ...
     * @param <T>  The type of the task's result.
     */
    public static <T> void executeByCustom(final ExecutorService pool, final ThreadUtils.Task<T> task) {
        execute(pool, task);
    }
     ...
     */
    public static void cancel(final ThreadUtils.Task task) {
        if (task == null) {
            return;
        }
        task.cancel();
     ...
     */
    public static void cancel(final ThreadUtils.Task... tasks) {
        if (tasks == null || tasks.length == 0) {
            return;
        }
        for (ThreadUtils.Task task : tasks) {
            if (task == null) {
                continue;
            }
            task.cancel();
     ...
     * @param tasks The tasks to cancel.
     */
    public static void cancel(final List<ThreadUtils.Task> tasks) {
    ...
    public static void cancel(ExecutorService executorService) {
        if (executorService instanceof ThreadUtils.ThreadPoolExecutor4Util) {
            for (Map.Entry<ThreadUtils.Task, ExecutorService> taskTaskInfoEntry : TASK_POOL_MAP.entrySet()) {
    ...
    private static <T> void execute(final ExecutorService pool, final ThreadUtils.Task<T> task) {
    ...
    private static ExecutorService getPoolByTypeAndPriority(final int type, final int priority) {
        synchronized (TYPE_PRIORITY_POOLS) {
            ...
            if (priorityPools == null) {
                ...
                pool = ThreadUtils.ThreadPoolExecutor4Util.createPool(type, priority);
    ...
    static final class ThreadPoolExecutor4Util extends ThreadPoolExecutor {
        ...
        private ThreadUtils.LinkedBlockingQueue4Util mWorkQueue;

        ThreadPoolExecutor4Util(int corePoolSize, int maximumPoolSize,
                                long keepAliveTime, TimeUnit unit,
                                ThreadUtils.LinkedBlockingQueue4Util workQueue,
        ...
        private static ExecutorService createPool(final int type, final int priority) {
            switch (type) {
                case TYPE_SINGLE:
                    return new ThreadUtils.ThreadPoolExecutor4Util(1, 1,
                            0L, TimeUnit.MILLISECONDS,
                            new ThreadUtils.LinkedBlockingQueue4Util(),
                            new ThreadUtils.UtilsThreadFactory("single", priority)
                    );
                case TYPE_CACHED:
                    return new ThreadUtils.ThreadPoolExecutor4Util(0, 128,
                            60L, TimeUnit.SECONDS,
                            new ThreadUtils.LinkedBlockingQueue4Util(true),
                            new ThreadUtils.UtilsThreadFactory("cached", priority)
                    );
                case TYPE_IO:
                    return new ThreadUtils.ThreadPoolExecutor4Util(2 * CPU_COUNT + 1, 2 * CPU_COUNT + 1,
                            30, TimeUnit.SECONDS,
                            new ThreadUtils.LinkedBlockingQueue4Util(),
                            new ThreadUtils.UtilsThreadFactory("io", priority)
                    );
                case TYPE_CPU:
                    return new ThreadUtils.ThreadPoolExecutor4Util(CPU_COUNT + 1, 2 * CPU_COUNT + 1,
                            30, TimeUnit.SECONDS,
                            new ThreadUtils.LinkedBlockingQueue4Util(true),
                            new ThreadUtils.UtilsThreadFactory("cpu", priority)
                    );
                default:
                    return new ThreadUtils.ThreadPoolExecutor4Util(type, type,
                            0L, TimeUnit.MILLISECONDS,
                            new ThreadUtils.LinkedBlockingQueue4Util(),
                            new ThreadUtils.UtilsThreadFactory("fixed(" + type + ")", priority)
                    );
            }
        }
    ...
    private static final class LinkedBlockingQueue4Util extends LinkedBlockingQueue<Runnable> {
        ...
        private volatile ThreadUtils.ThreadPoolExecutor4Util mPool;
    ...
    public abstract static class SimpleTask<T> extends ThreadUtils.Task<T> {
    ...
    public abstract static class Task<T> implements Runnable {
        ...
        private ThreadUtils.Task.OnTimeoutListener mTimeoutListener;
         ...
         */
        public ThreadUtils.Task<T> setTimeout(final long timeoutMillis, final ThreadUtils.Task.OnTimeoutListener listener) {
        ...
        public ThreadUtils.Task<T> setDeliver(Executor deliver) {
